...
t_glob		*eat_fct(int ind, t_glob *gen)
{
	...
	if (gen->philo->start == -1)
		gen->philo->start = gen->time_current - (gen->time_current % gen->time_eat);
	gen->philo->start_eat = gen->time_current - (gen->time_current % (gen->time_eat + gen->time_sleep)) + gen->philo->start; // gen->time_current;
